...
const stats = computed(() => reservations.value.map(r => {
  ...
  const bookedSeats = resBookings.reduce((sum: number, b: any) => sum + b.seat_count, 0)
  const paidCount = resBookings.filter((b: any) => b.payment_status === 'paid').length
  return {
    reservation: r,
    bookedSeats,
    paidCount,
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      ...
      <UButton to="/admin/reservations/new" icon="i-lucide-plus">
        Neue Reservierung
      </UButton>
    </div>
    ...
    <div v-else-if="reservations.length === 0" class="text-center py-16 text-muted">
      <div class="text-4xl mb-4">📋</div>
      <p class="mb-4">Noch keine Reservierungen angelegt.</p>
      <UButton to="/admin/reservations/new" icon="i-lucide-plus">
