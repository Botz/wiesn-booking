...
watch(codeData, (data) => {
  if (data?.reservations) {
    // Pre-select only if a single reservation exists
    if (data.reservations.length === 1) {
      selectedReservationIds.value = [data.reservations[0].id]
    }
  }
}, { immediate: true })
...
<template>
  <div class="max-w-2xl mx-auto px-4 py-8">
    <!-- Gate closed -->
    <div v-if="!isOpen" class="text-center py-16">
      <div class="text-5xl mb-4">🍺</div>
      <h1 class="text-2xl font-bold mb-2">Noch nicht verfügbar</h1>
      <p class="text-muted mb-8">Die Reservierung öffnet am 1. Mai um 08:00 Uhr</p>
    ...
    <!-- Loading -->
    <div v-else-if="codeLoading" class="flex justify-center py-16">
      ...
      <div class="mb-8 space-y-3">
        ...
        >
          <div class="flex justify-between items-start gap-3 mb-3">
            ...
            <ReservationWhatsAppButton v-if="r.whatsapp_link" :link="r.whatsapp_link" @click.stop />
          </div>
